...
const restaurantInfoCSV = fs.readFileSync(__dirname + '/../dataset/restaurants_info.csv', 'utf8');
const parsedCSV = Papa.parse(restaurantInfoCSV, { header:true, dynamicTyping: true });


// read the data from restaurant list and then match it to the parsed CSV file.
fs.readFile(__dirname + '/../dataset/restaurants_list.json', 'utf8', (error, restaurantList) => {
    ...
    fs.writeFile('./dataset/combinedData.json', JSON.stringify(combinedData), 'utf8', (error) => {

        if (error) {
            throw new Error('error creating new json file which contains combined the data');
        } else {
            console.log('Data has been combined! :tada:');
        }
    })
});
